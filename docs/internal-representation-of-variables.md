...
between two lvals of the same type, and that change over a value's lifetime:

FlagMeaningisNullThis value is currently null.isUndefinedThis slot currently holds undefined (absent).
...
CapacityDeclared asCurrent-state checkWhynullable?isNull flaga null has no content to carry a typeerrorable!derived: currentType <: erroran error has content, a real error-typed payload
...
4.1 Interning and runtime growth

Any type ever placed in the typetable must be interned: the same structural
type resolves to the same id every time. Without interning, repeated construction of
the "same" type would append duplicates and grow the table without bound.

Throwing is not a source of new types. try surfaces an error into a declared
result type (§3.1), T | error, static and written at the binding, while the
value's current type is a concrete error subtype, itself a statically-defined class.
Neither is synthesized on throw, so a throw appends nothing. With errors handled this
way, the typetable is bounded by the program's statically-declared types: a value's
current type is always one that already exists in the table.
...
lvals are collected by the Go garbage collector. Types that own internal
