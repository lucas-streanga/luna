...
The 56 id bits hold a **single** id: the value's **current** (concrete) type. A value
in motion carries only what it currently *is* — `int`, `string`, a specific error
subtype — never a union. Unions exist only as *declared* types, and a declared type
is a property of the **location** a value occupies, not of the value:
