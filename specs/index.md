...
| std.filesystem | `filesystem.md` | The structure module (R135): the `path` constraint + pure path ops, `exists`/`stat`/`entries`/`walk`, create/delete/rename/copyFile, temp files — under `filesystem`; the ioError family extended. |
...
| std.exec | `exec.md` | The command-running module (relocated from concurrency/, R172): the `exec` capability + free `run`/`capture` over the built-in `command` type — built-in type, std-module effect, the `secret`/`revealSecret` shape; pipefail always; the `@commandResult`/`commandError` split. |
| std.binary | `binary.md` | The binary-encoding module (R187): the exported `endian` enum + six endian-explicit fixed-width reads over `bytes` (`readI16`…`readU64`, width in the name, no size enum); pure, comptime-eligible; pulls `uint` (né `u64`, R226) + the 16/32 constraints into alpha; writes deferred. |
