...
| std.binary | `binary.md` | The binary-encoding module (R187): the exported `endian` enum + six endian-explicit fixed-width reads over `bytes` (`readI16`…`readU64`, width in the name, no size enum); pure, comptime-eligible; pulls `u64` + the 16/32 constraints into alpha; writes deferred. |
