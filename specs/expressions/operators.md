...
| `t apply P(name: v)` | structure | apply | static protocol application: `@P`-typed result, initializers checked at compile time, never errorable; dynamic application is the free function `applyDynamic()`, removal its inverse `unapply()` (protocols §4.6) | protocols §4 |
