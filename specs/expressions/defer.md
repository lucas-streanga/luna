...
`defer` is that block and nothing else, and needs no rule to make it so: a variant literal opens
`.{` (grammar §0.4, R272), so `defer .{read};` and `defer { x; };` cannot be confused.
