...
| `apply` | protocol application: the expression operator (`[] apply P(name: v)`, `@P`-typed, never errorable; **infix, tier 1a** — associativity §1, R158, not the tier-12 default) **and** the requirement declaration inside a `proto` block (`apply otherProto;`). The dynamic form is the free function `applyDynamic()` — named apart precisely because `apply` is reserved and so cannot be a callee (R265); its inverse `unapply()` is likewise a free function (protocols §4.6), no keyword at all | protocols §4, §7 |
...
`duration`, `instant` — committed with `std.time`, R132; `uint` (`u64` until R226) and the `i16`/`u16`/`i32`/`u32`
constraints, pulled into alpha by std.binary, R187; `nat`, the non-negative `int`
constraint, R226 — and `decimal`/`rational`/`complex`,
