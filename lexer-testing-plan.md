...
**Built: 71 files — 54 that lex clean, 17 under `testdata/error_producing/` — covering 127 of
§0's 133 tokens.** The six absent are exactly the R246
multi-line tokens the lexer cannot yet emit (`MARGIN`, `RAW_TEXT`, the four `TRIPLE_*`
delimiters), so there is no unexplained gap; diagnostics reach `L0003`, `L0004`, `L0007`–`L0012`.
...
- **Escape validation is unimplemented, so `L0005`, `L0006`, and `L0013` are never raised.**
  `lexEscape` consumes a backslash pair without consulting string §5.1's per-context table, so
  `"\q"` and `"\u{}"` and `"\u{D800}"` all lex clean. This is **R243's open item** — which phase
  validates escapes — surfacing as missing behaviour rather than as a wrong decision, and it is
  not a lexer bug until that question is ruled. `mode-malformed-codepoint.lex` pins the
  tokenization R245 does settle and carries an in-band `//` comment saying why no diagnostic
  follows, so the golden cannot be mistaken for a ruling that none is owed.
