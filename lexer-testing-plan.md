...
**Built: 87 files — 65 that lex clean, 22 under `testdata/error_producing/` — covering all 133 of
§0's tokens.** (The six R246 multi-line kinds are exercised only as inventory, the
lexer not emitting them yet.) Diagnostics reach every code the lexer can raise: `L0003`–
`L0013` and `L0016`, with `L0001`/`L0002` belonging to ingress and `L0014`/`L0015` to R246.
