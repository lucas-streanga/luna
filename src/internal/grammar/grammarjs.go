...
//
// Ordering inside an alternation is not load-bearing here, unlike TextMate. tree-sitter's
// lexer takes the longest match across every token rule, so `0x10` beats `0` whatever order
// the alternatives sit in. §0's order is kept anyway — it is longest-first already, so the
// file reads correctly under either rule and does not depend on knowing which applies.
func buildJS(p *patterns) *jsGrammar {
	g := &jsGrammar{}

	// Trivia. LINE_COMMENT transcribes; BLOCK_COMMENT cannot — §0 writes it `(?s)/\*.*?\*/`
