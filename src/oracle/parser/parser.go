...
// compare lexeme text.
//
// toks is the **full** stream — the parser walks the filtered view, and splice needs the rest.
//
...
func Parse(f *source.File, toks []token.Token) (*Tree, []diagnostic.Diagnostic) {
	...
	return build(f, toks, splice(toks, evs)), diags
}

// parse is the recursive descent itself: a function per grammar.md §0 nonterminal over the
// trivia-filtered view of toks. On damaged input it follows §7 — synthesise at an expect-site,
...
func parse(f *source.File, toks []token.Token) (eventStream, []diagnostic.Diagnostic) {
