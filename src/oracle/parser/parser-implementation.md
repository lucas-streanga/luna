...
happens where the panic is written. `oracle/diagnostic/panic_test.go` enforces the spelling over the whole
oracle rather than by memory, and its count of remaining `is unimplemented` sentinels is this
phase's progress bar.
