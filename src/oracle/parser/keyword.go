...
// isPathSegment asks the token's category rather than restating §0.9's list, which is safe
// because `internal/ebnf` already pins that every terminal §0 names is a lexer §0 token, and that
// §0.9's count matches lexer §10's (§10). A keyword added to the lexer and forgotten in §0.9 fails
