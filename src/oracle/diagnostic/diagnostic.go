...
//
// Compiler §3.1 (R240) fixes the shape. Every diagnostic carries a code, a one-letter
// stage prefix and four digits, because prose churns and tests must pin
...
// leaving the process (the language server), and because ingress failures like invalid
// UTF-8 or a leading BOM describe a file that never became a valid one. Resolving a
...
// Title is the phrase fixed to the code, derived and never stored, so a diagnostic
