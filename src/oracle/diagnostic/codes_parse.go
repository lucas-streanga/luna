...
//
// §11.2 lists ten diagnostics and numbers one of them, which is the point rather than an
// oversight: R250, R251 and R267 all rule that a code allocated before there is a check to raise
// it and a test to pin it is a code nothing checks. Worse than useless, in fact — a published code
// is a promise the compiler can produce it, and `luna explain` would be answering for a diagnostic
// that cannot occur.
//
// The nine unnumbered rows are not forgotten, they are waiting: §11.2 carries them with an em dash
// in the code column, TestParseCodesMatchSpec counts them, and each takes its number when the
